...
- SQLite schema v9, atomic migrations, 50-row keyset projections, external-content FTS, 64KiB reader excerpts, transactional local mutations, persistent mailbox statistics, remote identity storage, and a dedicated single-connection actor are implemented, tested, and constructed by `main` in a private per-user data directory.
...
- A file-backed vertical test now drives the production Slint controller, bounded core, SQLite actor, projections, FTS, and local mutations through success, empty, and repairable failure states. Fresh release-memory coverage remains required before protocol work; IMAP/SMTP, optional JMAP, MIME ingestion, OAuth2, and keyring integration remain planned backend work.
