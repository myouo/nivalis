...
- SQLite schema v10, atomic migrations, 50-row keyset projections, external-content FTS, 64KiB reader excerpts, transactional local mutations, persistent mailbox statistics, content generations, remote identity storage, and a dedicated single-connection actor are implemented, tested, and constructed by `main` in a private per-user data directory.
