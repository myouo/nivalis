...
The historical pre-controller-cutover X11 baseline is schema v8 at revision `f639c4b`, built as a 19,506,296-byte stripped release. Across three fresh 1200x900 Skia-software runs, the worst stable sample was 37.80 MiB RSS / 24.02 MiB PSS / 20.78 MiB USS. A separate 300-second idle soak held RSS exactly at 37.61 MiB from 30 seconds onward and returned to 0.00% interval CPU. A 10,000-action, five-minute run settled at 42.10/28.39/25.16 MiB, or +11.93%/+18.80%/+21.75%, while a 3840x2400 resize and restore settled at +80.71%/+63.54%/+0.11%. These results prove only that historical build against the idle and growth gates. The SQLite-controller cutover requires a fresh gate, and later IMAP/JMAP, MIME, attachment, and representative multi-account synchronization paths each require new coverage before their releases can inherit a pass.
