...
- SQLite schema v7, atomic migrations, 50-row keyset projections, 64KiB reader excerpts, transactional local mutations, persistent mailbox statistics, remote identity storage, and a dedicated single-connection actor are implemented, tested, and constructed by `main` in a private per-user data directory.
...
- Schema v7 provides bounded storage for mailbox-scoped IMAP locators, opaque account-scoped JMAP object states, compacted desired-state intents, frozen folder/locator snapshots, and versioned lease metadata. Local flags, placement changes, and terminal deletions reduce into this journal in the same immediate transaction as message state, undo/tombstone data, file-GC references, and statistics. Parent, child, and payload caps roll back the complete mutation on overflow; recursive triggers keep the constant-time child usage counter consistent across replacement and cascade paths. Legacy revisions retain indexed target-level reconciliation markers instead of unbounded migration backfill. The actor claims one complete intent through a cancellable oneshot path that bypasses UI reply backpressure; provider writes remain disabled until fenced report handling exists.
...
- The visible controller still reads and mutates one `MailStore` instance and deliberately does not consume SQLite mutation events. It will not switch individual rows to SQLite until claim/report integration, presentation mapping, error feedback, and FTS are complete; this avoids two conflicting sources of truth and prevents a later sync from overwriting offline changes.
...
A separate blocking thread owns the single `rusqlite::Connection`; neither the UI nor Tokio tasks access it directly. Crossbeam provides bounded requests and lifecycle signals, while bounded Tokio replies wake the core directly without another thread. The actor performs migrations, mailbox queries, local mail transactions, and remote-intent claiming serially; remote reports and SMTP outbox draining are not implemented yet. Submission and shutdown share an admission gate, so a request cannot report success after draining begins. FTS and remote journal execution remain on this same actor rather than adding persistence threads.
...
The latest measured X11 baseline is revision `0d19797` (schema v1, before the transactional mutation engine). It includes the active SQLite actor and asynchronous reply path. Across three fresh 1200x900 runs, the worst stable sample was 37.0 MiB RSS / 24.6 MiB PSS / 21.2 MiB USS with 0.00% interval CPU. Compared with the earlier UI-only baseline, the observed increase was about 1.3/2.8/2.8 MiB respectively, within the 4 MiB activation budget. The process exposed `nivalis-core` and `nivalis-sqlite` as its only application-owned background execution threads. The last verified schema-v6 release binary is 19,342,200 bytes; schema v7 requires a fresh release build and memory measurement before release gating.
