...
- Schema v7 provides bounded storage for mailbox-scoped IMAP locators, opaque account-scoped JMAP object states, compacted desired-state intents, frozen folder/locator snapshots, and versioned lease metadata. Parent and child caps are hard schema bounds, recursive triggers keep the constant-time child usage counter consistent across replacement and cascade paths, and legacy revisions use indexed target-level reconciliation markers instead of unbounded migration backfill. Provider writes remain disabled until local mutations update the journal atomically and the actor exposes bounded claim/report operations.
...
- The visible controller still reads and mutates one `MailStore` instance and deliberately does not consume SQLite mutation events. It will not switch individual rows to SQLite until journal mutation/claim integration, presentation mapping, error feedback, and FTS are complete; this avoids two conflicting sources of truth and prevents a later sync from overwriting offline changes.
