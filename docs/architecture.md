...
- The visible controller still reads and mutates one `MailStore` instance and deliberately does not consume SQLite mutation events. It will not switch individual rows to SQLite until provider execution, synchronization merge/reconciliation, presentation mapping, error feedback, and FTS are complete; this avoids two conflicting sources of truth and prevents a later sync from overwriting offline changes.
