...
- The visible controller now starts with empty Slint models and consumes bounded SQLite account, mailbox, statistic, and selected-detail projections as its sole source of truth. First, Next, and Previous requests use bidirectional keyset cursors; the controller retains only the current page, at most 50 visible message IDs, two cursors, and a page number rather than page history. Request ID and generation fencing reject obsolete pages and details. Local mutation controls remain explicitly read-only until ordered mutation events and undo are connected; the in-memory repository is test-only.
...
- Controller mutation/undo routing, IMAP/SMTP, optional JMAP, MIME ingestion, OAuth2, and keyring integration remain planned backend slices.
...
The current local-read and pagination X11 gate is schema v8 at release-code revision `0d3453c`, built as a 19,410,424-byte stripped production release. Its checked fixture reaches the 64-account catalog, a 50-row page plus one-row continuation, 2KiB preview, and 64KiB reader-excerpt bounds. Across three fresh 1200x900 Skia-software processes, RSS and VmHWM stayed at or below 38,408KiB (37.51MiB). Two independent 10,000-transition First/Next/Previous soaks settled at 38,364KiB and 38,336KiB RSS, only +0.68% and +0.75% from their pre-stress baselines; the largest measured VmHWM was 38,364KiB. This proves the 90MiB hard idle gate and less-than-2x growth for the current SQLite read and two-page navigation slice. Repeated normal runs meet the preferred 50MiB idle target, but one retained historical 68.62MiB outlier prevents an unconditional target claim. Controller writes, FTS, IMAP/JMAP, MIME, attachments, and representative multi-account synchronization each require fresh coverage when activated.
