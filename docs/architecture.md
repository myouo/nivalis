...
This document defines the final ownership and resource boundaries, not a requirement to finish every provider or hardening path before shipping the next usable slice. Milestone work takes the shortest path to a real end-to-end mail workflow and records non-blocking gaps in the roadmap instead of creating implementation-version ladders. M4 ships manually triggered, single-account, single-page IMAP receive over the same platform-verified Rustls and keyring boundary as M3; broader OAuth, automatic synchronization, and provider coverage follow without weakening the same limits. Persistent file reservations, directory-relative race hardening, secret-file open TOCTOU resistance, cross-platform ACL behavior, deep fuzzing, stronger cross-allocator clearing of secrets that briefly enter Slint `SharedString`, and automatic benchmark compensation after setup failures belong to M7; strict commit recovery for irreplaceable outbound data belongs to the M5 outbox.
...
- File-backed vertical tests drive the production Slint controller and bounded content lifecycle. The receive test crosses fake-keyring secret load, fenced page staging and content import, production mailbox/detail queries, body stream open/close, account purge, and bounded orphan collection; separate cases prove oversized rejection and idempotent partial retry. The opt-in M4 release benchmark crosses real Secret Service store/load/delete plus two bounded TLS/IMAP sessions and verifies exactly one import, open, close, and removal. Automatic paging, multiple accounts, real providers, reconnect/IDLE, and long-duration synchronization remain unmeasured.
...
The latest measured schema-v11 X11 checkpoint is release-code revision `d5a6c43ce7f5096cbb46052d1a477e0cc1db4063`. Three fresh production empty-idle runs peak at 32,556KiB RSS. One loopback account lifecycle exercises production add/diagnose, bounded INBOX metadata and body fetch, generation- and UIDVALIDITY-fenced staging, MIME import, body open/close, account removal, and delayed janitor recovery. It peaks and settles at 37,024KiB RSS; from the 10-second baseline to the 135-second sample it grows 12.67% RSS, 17.13% PSS, and 9.04% RSS+Swap, then records 0.00% CPU. These paths pass both the 90MiB hard gate and preferred 50MiB target. Host swap pressure is recorded rather than treated as an optimization, a retained historical 68.62MiB outlier still prevents an unconditional 50MiB guarantee, and this single-message loopback is not evidence for real providers, automatic paging, multiple accounts, outbound delivery, or a long-duration protocol soak.
