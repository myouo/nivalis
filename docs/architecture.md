...
- File-backed vertical tests drive the production Slint controller and bounded content lifecycle. The content test imports, opens, closes, replaces, and collects one message through the real actor and private file store. A separate account test creates through the core, reads the stored secret, constructs the post-delete crash window, restarts, proves mailbox queries remain responsive while the keyring is blocked, and observes final fenced purge. The `456ad2f` release matrix keeps the M2 local-content memory gate closed after the credential dependency was added; the connected warm credential path and provider connectivity remain unmeasured.
