...
- A file-backed vertical test drives the production Slint controller, bounded core, SQLite actor, projections, FTS, and local mutations through success, empty, and repairable failure states. The `a74b8bb` release matrix closes the local SQLite memory gate; bounded MIME ingestion is next, while IMAP/SMTP, optional JMAP, OAuth2, and keyring integration remain planned backend work.
...
The current schema-v9 SQLite X11 gate is release-code revision `a74b8bb`, built as a 19,432,824-byte stripped production release plus a separate opt-in bench binary. Its checked fixture reaches 64 accounts, 51 messages, a 50+1-row keyset boundary, 2KiB previews, 64KiB reader excerpts, and one deterministic FTS match. Three fresh 1200x900 Skia-software production processes stayed at or below 38,660KiB (37.75MiB) RSS. Two independent 1,000-cycle star-write/FTS/clear soaks and two 10,000-transition pagination soaks peaked at or below 39,100KiB RSS; every settled RSS, PSS, RSS+Swap, and PSS+SwapPss value remained below 2x baseline and every dedicated CPU window returned 0.00%. This closes the M1 local SQLite gate. A retained historical 68.62MiB outlier prevents an unconditional 50MiB guarantee; large upgrades, MIME, attachments, IMAP/JMAP, and representative multi-account synchronization require fresh milestone coverage.
