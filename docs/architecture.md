...
- SQLite schema v6, atomic migrations, 50-row keyset projections, 64KiB reader excerpts, transactional local mutations, persistent mailbox statistics, and a dedicated single-connection actor are implemented, tested, and constructed by `main` in a private per-user data directory.
...
- The visible controller still reads and mutates one `MailStore` instance and deliberately does not consume SQLite mutation events. It will not switch individual rows to SQLite until the durable remote-operation journal, presentation mapping, error feedback, and FTS are complete; this avoids two conflicting sources of truth and prevents a later sync from overwriting offline changes.
