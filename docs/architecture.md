...
A separate blocking thread owns the single `rusqlite::Connection`; neither the UI nor Tokio tasks access it directly. Crossbeam provides bounded requests and lifecycle signals, while bounded Tokio replies wake the core directly without another thread. The actor performs migrations, mailbox queries, and local mail transactions serially; the schema reserves a durable SMTP outbox and remote journal, but their drain/claim APIs are not implemented yet. Submission and shutdown share an admission gate, so a request cannot report success after draining begins. FTS and remote journal execution remain on this same actor rather than adding persistence threads.
