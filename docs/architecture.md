...
- The visible controller now starts with empty Slint models and consumes bounded SQLite account, mailbox, statistic, and selected-detail projections as its sole source of truth. A controller session retains at most 50 visible message IDs and rejects obsolete pages/details by request ID and generation. Local mutation controls remain explicitly read-only until ordered mutation events and undo are connected; the in-memory repository is test-only.
...
- Controller mutation/undo routing, keyset navigation, IMAP/SMTP, optional JMAP, MIME ingestion, OAuth2, and keyring integration remain planned backend slices.
...
The current local-read X11 gate is schema v8 at release-code revision `d19cec5`, built as a 19,382,904-byte stripped release. Its checked fixture reaches the 64-account catalog, 50-row page plus continuation, 2KiB preview, and 64KiB reader-excerpt bounds. Across three fresh 1200x900 Skia-software processes, RSS stayed at or below 37.60MiB. A real-row 10,000-action run settled at 45.05/32.16/28.34MiB RSS/PSS/USS, or +19.85%/+30.22%/+35.77%, while a production 3840x2400 resize and restore retained 68.53MiB RSS, or +82.91%/+65.36%/+4.60%. This proves the 90MiB hard idle gate and the less-than-2x growth gate for the current SQLite read slice. Repeated normal runs meet the preferred 50MiB idle target, but one retained 68.62MiB outlier prevents an unconditional target claim. Pagination, controller writes, FTS, IMAP/JMAP, MIME, attachments, and representative multi-account synchronization each require fresh coverage when activated.
