...
Recorded follow-ups that do not block M4: automatic paging and large-mailbox windowed UID discovery/ESEARCH; folders beyond INBOX; automatic UIDVALIDITY reset and cache rebuild; reconnect/backoff and bounded IDLE; explicit user cancellation; graceful handling of one oversized or malformed message without blocking later mail; outbound desired-state execution and full incoming/outgoing merge and reconciliation; and ambiguous MOVE/COPY recovery. M6 owns the scheduling and convergence work needed before multi-account synchronization is complete. Real-provider breadth, large mailboxes, repeated reconnects, and multi-hour soaks remain M7 release gates rather than reasons to delay the first usable receive slice.

## M5: drafts, outbox, and SMTP

Status: complete.

Checkpoint: one selected app-password account can load its latest durable plain-text draft after restart, save it behind account-generation and draft-revision fences, and queue it through the production UI. Closing a dirty composer, including a native window-close request, first performs the same bounded durable save and keeps the window open with actionable feedback if validation or storage fails. The core streams the 1MiB-bounded body and deterministic MIME into private files, records the outbound reservation before publication, and finalizes the artifact atomically into the SQLite outbox. The living schema is v13: the outbox was introduced in v12, while v13 corrects draft mailbox statistics without changing its delivery protocol.

One global drainer on the existing current-thread runtime claims at most one message, loads its credential only for the attempt, records a durable DATA fence after SMTP `354` and before the first body byte, retries safe pre-DATA failures with bounded 30-second-to-one-hour backoff, and marks ambiguous post-DATA outcomes `uncertain` rather than risking an automatic duplicate. The persistent Outbox projection retains at most 64 rows and exposes fenced retry, release, uncertain-result review, cancellation, and exact-generation SMTP credential repair. Shutdown rejects new attempts, cancels active protocol work, waits for bounded drainage, and preserves a retry-safe or uncertain durable state. Delivered and released artifacts wake a capacity-one janitor signal; the same runtime rechecks references and removes at most 16 private files per batch, including startup backlogs larger than one batch.

Focused tests cover reservation and action fencing, rollback, v1/v2 result merging, progress lease renewal, safe pre-DATA restart, ambiguous post-DATA disconnect, explicit SMTP 4xx handling, cancellation and shutdown, credential replacement at the same keyring locator, persistent Outbox actions, dirty-window-close persistence, and janitor progress when UI status hints are lost. The production account-send benchmark crosses the actual account setup, keyring, IMAP diagnostic, composer, private MIME artifact, Rustls/STARTTLS SMTP DATA, Sent projection, account removal, and same-process durable cleanup path.

Release-code revision `24d616f8b947965acc32998438608d7714eaaf19` closes the M5 memory gate. Three production idle runs peak at 38,760KiB RSS (37.85MiB). The loopback account-send lifecycle starts from a 38,772KiB ten-second RSS baseline, peaks and settles at 42,856KiB, and grows 10.53%; PSS grows from 28,930KiB to 32,538KiB, or 12.47%. Swap remains zero and the final dedicated CPU sample is 0.00%. After process exit, all account, outbox, content, staging, recipient, draft, and GC rows are absent, SQLite integrity is `ok`, the foreign-key check is empty, and no private content file remains. Both the 90MiB hard gate and preferred 50MiB target pass. The earlier SMTP-enabled idle-only checkpoint at `b852f10` remains retained historical evidence, but is no longer the M5 closure revision.
...
- SMTP submission supports bounded retry/backoff, authentication recovery, permanent failure, uncertain-result review, explicit production cancellation, and user-visible delivery state.
...
Recorded follow-ups that do not block M5: CC/BCC, attachments, HTML or rich-text composition, signatures, reply/forward, SMTP auto-discovery and provider presets, and OAuth2 remain product-coverage work. M6 owns fair multi-account scheduling and receive/send convergence. M7 retains generic content-store persistent reservation ownership and lease recovery, directory-relative `openat`/`unlinkat` race hardening, cross-platform ACL and reparse-point handling, deep fuzzing/Miri, delayed cancellation while loading credentials or files, a complete successful credential-repair UI lifecycle test, SMTP endpoint and certificate reconfiguration, focus-restoration and compact-badge verification, real-provider breadth, deep partial-sync failure cleanup, repeated reconnects, and multi-hour send/retry soaks. These do not justify subdividing the completed usable send slice.
...
Provider-compatibility checkpoint: revision `d663a94` replaces dense UID probing with bounded sparse `UID SEARCH`, accepts the omitted implicit UID seen in a single-UID body response while still rejecting an explicit mismatch, and treats `RFC822.SIZE` as metadata rather than requiring byte equality with a canonically transferred body literal. A real IMAP4rev1 account passed platform-trusted TLS and authentication, then production startup synchronization advanced its fenced cursor, persisted message content, passed SQLite integrity checks, and populated the production UI model. Revision `f5d3d7b` adds an opt-in existing-account harness that disables automatic scheduling before core startup, invokes the same manual-sync callback as the UI, requires its loading transition, and verifies a fresh timestamp, stable or initialized UID identity, monotonic cursor/count, one transactional SQLite snapshot, and exact first-page UI identities. Its live execution and release-memory sample remain pending external GUI/network authorization, so this checkpoint does not claim that an explicit manual real-account run or the current release memory gate has passed.

Remaining milestone work: execute the existing durable desired-state journal against IMAP, merge incoming provider state without overwriting newer local intent, make the SMTP claimant fair across account FIFO heads, add broader provider compatibility fixtures, and measure release idle plus settled multi-account growth. Optional bounded JMAP remains behind those provider-neutral convergence semantics and must not expand the default dependency tree. The current `async_imap` `UID SEARCH` API materializes the bounded-on-wire result set before Nivalis truncates it to one page; a large-mailbox streaming/windowed discovery strategy and its heap measurements remain an M7 hardening item.
