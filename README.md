...
The confirmed application stack is Rust 2024 with Slint 1.17.1, Winit, and the Skia renderer. The production architecture keeps the native Slint UI on the main thread, runs network work on one Tokio current-thread runtime, and serializes SQLite access through a dedicated `rusqlite` actor. Mail transport uses IMAP and SMTP, with JMAP as an optional provider backend; MIME parsing, OAuth2, Rustls, and the `keyring-core` ecosystem complete the protocol and credential boundary. Nivalis does not embed a WebView.
...
The current release checkpoint is `9f0fd17`, measured with a 19,926,744-byte production binary and the checked 64-account, 51-message fixture. Three fresh 1200x900 X11 processes peaked at 38,980KiB (38.07MiB) RSS. A production process that opened the real Secret Service and recovered one pending account removal stayed exactly at 39,448KiB RSS through 615 seconds. The 10,000-cycle MIME/body/attachment/GC workload peaked at 40,084KiB (39.14MiB), settled at +2.56% RSS/+3.43% PSS, used zero swap, and left only the current body and attachment. Every final CPU window was 0.00%. This proves the hard 90MiB gate and preferred 50MiB target for the measured paths, while a retained historical outlier still prevents an unconditional 50MiB guarantee. See [`memory-report.md`](memory-report.md) for raw samples, hashes, limitations, and reproduction commands.
