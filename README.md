...
The confirmed application stack is Rust 2024 with Slint 1.17.1, Winit, and the Skia renderer. The production architecture keeps the native Slint UI on the main thread, runs network work on one Tokio current-thread runtime, and serializes SQLite access through a dedicated `rusqlite` actor. The active receive boundary uses `async-imap`, `mail-parser`, Rustls, and the `keyring-core` ecosystem. SMTP, OAuth2, and optional JMAP activate only in their later measured milestones. Nivalis does not embed a WebView.

The bounded Tokio core, schema-v11 persistence layer, keyset mailbox projections, external-content FTS, and single-connection SQLite actor are active. Local flags, Archive, Trash, Trash undo, and permanent deletion run as immediate transactions that compact bounded desired remote state with undo, tombstones, deferred file cleanup, and persistent statistics. Schema v11 retains the v10 generation-fenced content lifecycle and adds a non-secret generation-fenced account configuration boundary. The local content pipeline bounds untrusted MIME work, streams normalized bodies and attachments into private files, opens them without materializing mailbox-wide buffers, and rechecks every SQLite reference before removal. The visible controller reads accounts, the current 50-row mailbox page, persistent counters, and one selected detail exclusively from SQLite; request IDs and view generations reject obsolete results. Users can add, diagnose, manually synchronize one bounded INBOX page, and remove a configured app-password IMAP account while its secret remains in the operating-system keyring. Automatic synchronization, folders beyond INBOX, multi-account scheduling, and outbound provider execution remain later milestones. See [`docs/architecture.md`](docs/architecture.md) for ownership boundaries, backpressure rules, dependency features, implementation status, and memory budgets.
...
- The active SQLite path returns at most 50 metadata rows, reads at most one 64KiB reader excerpt, stores large bodies and MIME payloads by private file reference, and caps the SQLite page cache at 1MiB. Mailbox pages include persistent counters and at most 64 account unread values without scanning for exact counts; raw import writes mark counters stale until an atomic rebuild completes. Local mutations serialize on the same actor, retain at most 256 folders in the single trash-undo slot, and queue zero-reference file keys for later deletion without loading file-key collections into Rust memory. Their remote journal snapshots remain in SQL and are capped at 4,096 targets per account, 16,384 globally, 65,536 child rows, and 320KiB per provider payload. The actor and Tokio core remain separate threads; no reply-bridge thread or periodic polling loop is created. First/Next/Previous keyset navigation retains only the current 50-row page and two cursors. Manual receive uses that same bounded core; outbound execution and full synchronization merge/reconciliation remain required.

The current release-code checkpoint is `d5a6c43ce7f5096cbb46052d1a477e0cc1db4063`. Three production empty-idle runs peak at 32,556KiB RSS. One loopback lifecycle drives add, diagnostic, bounded IMAP fetch, fenced MIME import, body open/close, and account removal; a following normal release start drains the delayed file-GC item. The receive run peaks and settles at 37,024KiB RSS, grows 12.67% RSS and 17.13% PSS from the 10-second baseline, and returns to 0.00% CPU. Both the 90MiB hard gate and preferred 50MiB target pass for this measured M4 slice. Host swap pressure is retained in the raw evidence, and this result is not a guarantee for real providers, automatic paging, large mailboxes, multiple accounts, or long-duration synchronization. See [`memory-report.md`](memory-report.md) for raw samples, hashes, limitations, and reproduction commands.

This is now a minimal receiving mail client: one selected app-password IMAP account can fetch and display one bounded INBOX page through the production stack. It is not yet an automatic, full-folder, multi-account synchronization client and cannot send mail. M5 owns durable compose/outbox/SMTP; M6 owns bounded multi-account scheduling and convergence. Simulated delivery success remains intentionally absent.
