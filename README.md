...
The bounded Tokio core, schema-v9 persistence layer, keyset mailbox projections, external-content FTS, and single-connection SQLite actor are now active. Local flags, Archive, Trash, Trash undo, and permanent deletion run as immediate transactions that compact the latest desired remote state with bounded undo, tombstones, deferred file cleanup, and persistent mailbox-statistic deltas in the same commit. Schema v9 retains mailbox-scoped IMAP locators, account-scoped JMAP state, a bounded desired-state journal, versioned lease metadata, and placement-rebase capacity reservations. The actor exposes bounded account-directory, mailbox, and reader projections plus fully fenced remote claim/report transactions. SQLite replies enter the core through a bounded asynchronous channel; account, mailbox, and selected-message results use independent latest-value slots, while mutation results remain ordered control events and accepted writes are drained during shutdown. The visible controller reads accounts, the current 50-row mailbox page, persistent counters, and one selected detail exclusively from SQLite. It routes local flags, Archive, Trash, permanent deletion, and absolute-deadline Trash undo through the ordered mutation path, then accepts only an authoritative first-page refresh before enabling another ordinary action. Request IDs and view generations reject obsolete results, and an empty database shows an honest no-account state. Provider execution remains disabled until its protocol milestone. See [`docs/architecture.md`](docs/architecture.md) for ownership boundaries, backpressure rules, dependency features, implementation status, and memory budgets.
...
- Search uses a restartable 180ms debounce, a 256-byte input cap, generation rejection, schema-v9 external-content SQLite FTS, and exact-key interruption of obsolete mailbox work.
...
The current schema-v9 SQLite-controller gate was measured at release-code revision `a74b8bb` with a 19,432,824-byte production binary and a checked 64-account, 51-message, 50+1-page fixture with 2KiB previews, 64KiB details, and a unique FTS hit. Three fresh 1200x900 X11 production processes peaked at 38,660KiB (37.75MiB) RSS and returned to 0.00% CPU. Two independent 1,000-cycle star-write/single-hit-FTS/clear soaks and two 10,000-transition pagination soaks stayed below 39,100KiB observed peak RSS; their settled RSS+Swap and PSS+SwapPss totals remained well below 2x baseline. This proves the hard 90MiB gate and the preferred 50MiB target for this matrix, while a retained 68.62MiB historical outlier still prevents an unconditional 50MiB guarantee. See `memory-report.md` for raw samples, hashes, swap-aware growth, limitations, and reproduction commands.

This is a persistent local-cache mail manager with production runtime and storage foundations, not yet a connected mail client. Simulated synchronization and delivery success are intentionally absent. M1 is complete: SQLite is the sole source of visible mailbox truth, the production-path controller test covers accounts, keyset pages, details, FTS, local writes, Trash undo, permanent deletion, empty storage, and repairable cache failure, and the release memory matrix covers idle, representative star writes with a fixed single-hit FTS query, and pagination. M2 is the bounded MIME and local-content pipeline; provider connectivity remains a later milestone.
