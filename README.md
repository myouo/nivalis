...
The current schema-v8 SQLite-controller gate was measured at release-code revision `d19cec5` with a 19,382,904-byte production binary and a fully checked bounded fixture: 64 accounts, 51 messages, a full 50-row page plus continuation, 2KiB previews, and 64KiB details. Three fresh 1200x900 X11 processes stayed at or below 37.60MiB RSS. A real-row 10,000-action run settled at 45.05/32.16/28.34MiB RSS/PSS/USS, or +19.85%/+30.22%/+35.77%; a production 3840x2400 resize/restore retained 68.53MiB RSS, or +82.91%. This proves the 90MiB hard gate and less-than-100% growth gate for the local-read slice. Repeated normal-idle runs meet the preferred 50MiB target, but a retained 68.62MiB historical outlier prevents treating that target as an unconditional guarantee. See `memory-report.md` for raw samples, hashes, limitations, and reproduction commands.

This is a persistent local-cache reader with production runtime and storage foundations, not yet a connected mail client. Simulated synchronization and delivery success are intentionally absent. SQLite is now the sole source of visible mailbox truth; the remaining M1 work is keyset navigation, controller mutation/undo routing, bounded FTS with interruption, and controller integration coverage. The release memory gate must be rerun after those paths activate and before provider work resumes.
