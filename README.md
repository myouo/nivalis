...
The bounded Tokio core, schema-v5 persistence layer, keyset mailbox projections, and single-connection SQLite actor are now active. Local flag and folder changes run as immediate transactions with bounded trash undo, tombstones, and deferred file cleanup. SQLite replies enter the core through a bounded asynchronous channel; mailbox pages and selected-message details coalesce by request generation, while mutation results remain ordered control events and accepted writes are drained during shutdown. The controller still uses the in-memory repository until the durable remote-operation journal, account statistics, presentation/error mapping, and FTS reach parity, so the visible app remains an interaction-complete local prototype with one consistent UI data source. See [`docs/architecture.md`](docs/architecture.md) for ownership boundaries, backpressure rules, dependency features, implementation status, and memory budgets.
...
- The active SQLite path returns at most 50 metadata rows, reads at most one 64KiB reader excerpt, stores large bodies and MIME payloads by private file reference, and caps the SQLite page cache at 1MiB. Local mutations serialize on the same actor, retain at most 256 folders in the single trash-undo slot, and queue zero-reference file keys for later deletion without loading file-key collections into Rust memory. The actor and Tokio core remain separate threads; no reply-bridge thread or periodic polling loop is created. A durable remote-operation journal, persistent statistics, FTS, and active-query interruption are still required before this path replaces the live UI repository.

The latest measured Linux release baseline is revision `0d19797` (schema v1, before the transactional mutation engine) with an 18.4MiB executable. With the SQLite actor active, three fresh X11 runs at 1200x900 produced a worst stable idle sample of 37.0MiB RSS / 24.6MiB PSS / 21.2MiB USS and 0.00% interval CPU. A 2,000-action run settled at 41.1/27.3/23.7MiB, while a 3840x2400 resize and restore settled at 66.7/39.3/20.9MiB; both remain below the 100% growth limit. Schema v5 must be remeasured before these figures are treated as its release gate. See `memory-report.md` for the measurement contract, historical platform references, and reproduction commands.

This is an interaction-complete local prototype with production runtime and persistence foundations. The next backend slice records local changes in a bounded, durable remote-operation journal so synchronization cannot overwrite offline work. Persistent account statistics and FTS then complete the consistent repository boundary before the controller switches away from its in-memory implementation. IMAP/SMTP, optional JMAP, MIME ingestion, and encrypted credentials follow that cutover.
