...
The bounded Tokio core, schema-v8 persistence layer, keyset mailbox projections, and single-connection SQLite actor are now active. Local flags, Archive, Trash, Trash undo, and permanent deletion run as immediate transactions that compact the latest desired remote state with bounded undo, tombstones, deferred file cleanup, and persistent mailbox-statistic deltas in the same commit. Schema v8 stores mailbox-scoped IMAP locators, account-scoped JMAP state, a bounded desired-state journal, versioned lease metadata, and placement-rebase capacity reservations. The actor now exposes bounded account-directory, mailbox, and reader projections plus fully fenced remote claim/report transactions. SQLite replies enter the core through a bounded asynchronous channel; account, mailbox, and selected-message results use independent latest-value slots, while mutation results remain ordered control events and accepted writes are drained during shutdown. Provider execution is deliberately not connected yet. The controller still uses the in-memory repository while the M1 SQLite presentation cutover is completed, so the visible app is an explicitly labeled local prototype with one consistent UI data source. See [`docs/architecture.md`](docs/architecture.md) for ownership boundaries, backpressure rules, dependency features, implementation status, and memory budgets.
...
- Read, star, archive, and mark-unread prototype flows; unavailable delivery actions are not exposed
- Confirmed trash actions, permanent deletion, and a timed undo snackbar
- Static loading rows, account error, empty/search-empty, disabled, and error states
...
- Message previews are capped at 280 Unicode scalar values, and the local reader shapes at most 16,384 values. Full-body loading stays unavailable until the bounded file-content pipeline is connected.
...
This is a local UI prototype with production runtime and persistence foundations, not yet a connected mail client. Simulated synchronization and in-memory delivery success are intentionally absent. Local mutations, versioned single-intent claims, and fenced reports form one bounded durable SQLite boundary; the current milestone is replacing the visible in-memory repository with that boundary before provider work resumes.
