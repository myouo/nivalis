...
The bounded Tokio core, schema-v7 persistence layer, keyset mailbox projections, and single-connection SQLite actor are now active. Local flag and folder changes run as immediate transactions with bounded trash undo, tombstones, deferred file cleanup, and persistent mailbox-statistic deltas in the same commit. Schema v7 adds mailbox-scoped IMAP locators, account-scoped JMAP state, and a bounded desired-state journal with versioned lease metadata; the mutation reducer and claim/report actor API are not implemented yet. SQLite replies enter the core through a bounded asynchronous channel; mailbox pages and selected-message details coalesce by request generation, while mutation results remain ordered control events and accepted writes are drained during shutdown. The controller still uses the in-memory repository until journal integration, presentation/error mapping, and FTS reach parity, so the visible app remains an interaction-complete local prototype with one consistent UI data source. See [`docs/architecture.md`](docs/architecture.md) for ownership boundaries, backpressure rules, dependency features, implementation status, and memory budgets.
...
- The active SQLite path returns at most 50 metadata rows, reads at most one 64KiB reader excerpt, stores large bodies and MIME payloads by private file reference, and caps the SQLite page cache at 1MiB. Mailbox pages include persistent counters and at most 64 account unread values without scanning for exact counts; raw import writes mark counters stale until an atomic rebuild completes. Local mutations serialize on the same actor, retain at most 256 folders in the single trash-undo slot, and queue zero-reference file keys for later deletion without loading file-key collections into Rust memory. The remote journal is capped at 4,096 targets per account, 16,384 globally, and 65,536 child snapshots through a constant-time usage counter. The actor and Tokio core remain separate threads; no reply-bridge thread or periodic polling loop is created. Journal mutation/claim integration, FTS, and active-query interruption are still required before this path replaces the live UI repository.

The latest measured Linux release baseline is revision `0d19797` (schema v1, before the transactional mutation engine) with an 18.4MiB executable. With the SQLite actor active, three fresh X11 runs at 1200x900 produced a worst stable idle sample of 37.0MiB RSS / 24.6MiB PSS / 21.2MiB USS and 0.00% interval CPU. A 2,000-action run settled at 41.1/27.3/23.7MiB, while a 3840x2400 resize and restore settled at 66.7/39.3/20.9MiB; both remain below the 100% growth limit. The last verified schema-v6 release binary builds at 19,342,200 bytes; schema v7 requires a fresh release build and memory measurement before these historical figures are treated as its release gate. See `memory-report.md` for the measurement contract, historical platform references, and reproduction commands.

This is an interaction-complete local prototype with production runtime and persistence foundations. The next backend slice merges local mutations into the bounded durable journal in the same SQLite transaction, followed by its versioned claim/report actor API. FTS, presentation mapping, and explicit error feedback then complete the consistent repository boundary before the controller switches away from its in-memory implementation. IMAP/SMTP, optional JMAP, MIME ingestion, and encrypted credentials follow that cutover.
