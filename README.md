...
The current release checkpoint is `528c2b440db1c731110b12e88bc9b532d1a52d16`. Three production empty-idle runs peak at 36,792KiB RSS. One loopback lifecycle drives the production UI add/status/remove path, real Secret Service store/load/delete, SQLite generation fencing, and bounded Rustls/IMAP TLS, LOGIN, CAPABILITY, EXAMINE, and best-effort LOGOUT; it peaks at 38,988KiB RSS, remains stable through the 135-second settled sample, grows 5.72% RSS/5.35% PSS, and uses zero swap. This proves the hard 90MiB gate and preferred 50MiB target for the measured M3 slice, not retained-account idle, repeated diagnostics, multiple accounts, a real provider, synchronization, or a long-duration protocol soak. A retained historical outlier still prevents an unconditional 50MiB guarantee. See [`memory-report.md`](memory-report.md) for raw samples, hashes, limitations, and reproduction commands.

This is a persistent local-cache mail manager with a completed app-password account-diagnostic slice, not yet a receiving mail client. Simulated synchronization and delivery success are intentionally absent. M1 established SQLite as the sole source of visible mailbox truth, M2 completed the bounded local MIME/file lifecycle, and M3 completed add/diagnose/remove across Slint, SQLite, Secret Service, Rustls, and IMAP. M4 is the next milestone and owns real receive synchronization.
