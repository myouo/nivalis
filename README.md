...
This is a persistent local-cache mail manager with production runtime and storage foundations, not yet a connected mail client. Simulated synchronization and delivery success are intentionally absent. SQLite is the sole source of visible mailbox truth; a production-path controller test now covers accounts, keyset pages, details, FTS, local writes, Trash undo, permanent deletion, empty storage, and repairable cache failure. The remaining M1 work is fresh release-memory coverage of the activated write/search paths. Provider work remains paused until that gate passes.
