...
This is a persistent local-cache reader with production runtime and storage foundations, not yet a connected mail client. Simulated synchronization and delivery success are intentionally absent. SQLite is now the sole source of visible mailbox truth; the remaining M1 work is controller mutation/undo routing, bounded FTS with interruption, and controller integration coverage. The release memory gate must be rerun after those paths activate and before provider work resumes.
