...
const MAX_METADATA_CONNECTIONS: usize = 4;
...
const MAX_METADATA_PREVIEW_WIRE_BYTES: usize = 8 * 1024;
// Some commercial IMAP servers ignore the partial FETCH length and return a
// larger literal. Keep enough of that already-transferred data to find text
// after MIME preambles, while retaining a strict client-side memory bound.
const MAX_PROVIDER_PREVIEW_FALLBACK_BYTES: usize = 128 * 1024;
const MAX_METADATA_PREVIEW_BYTES: usize = 2 * 1024;
const MAX_PREVIEW_CHARACTERS: usize = 512;
const MAX_PREVIEW_FETCH_MESSAGES: usize = 50;
...
const CLIENT_ID_COMMAND: &[u8] = concat!(
    "ID (\"name\" \"Nivalis\" \"version\" \"",
...
impl ImapInboxFetchRequest {
    ...
    pub(crate) fn requests_history(&self) -> bool {
        self.history_cursor.is_some() && !self.history_complete
    }
}
...
pub(crate) struct ImapPreviewFetchRequest {
    host: Box<str>,
    port: u16,
    login: Box<str>,
    secret: Secret,
...
impl ImapPreviewFetchRequest {
    pub(crate) fn new(
        ...
        uid_validity: u32,
        uids: &[u32],
    ) -> Result<Self, ImapPreviewFetchInputError> {
        validate_connection_input(host, port, login, &secret)
            .map_err(ImapPreviewFetchInputError::Connection)?;
        if uids.is_empty() || uids.len() > MAX_PREVIEW_FETCH_MESSAGES {
            return Err(ImapPreviewFetchInputError::Uids);
        }
        let mut validated = Vec::with_capacity(uids.len());
        for uid in uids {
            let uid = NonZeroU32::new(*uid).ok_or(ImapPreviewFetchInputError::Uids)?;
            if validated.contains(&uid) {
                return Err(ImapPreviewFetchInputError::Uids);
            }
            validated.push(uid);
        }
        Ok(Self {
            ...
            uid_validity: NonZeroU32::new(uid_validity)
                .ok_or(ImapPreviewFetchInputError::UidValidity)?,
            uids: validated.into_boxed_slice(),
        })
    }
}

impl fmt::Debug for ImapPreviewFetchRequest {
    fn fmt(&self, formatter: &mut fmt::Formatter<'_>) -> fmt::Result {
        formatter.write_str("ImapPreviewFetchRequest([REDACTED])")
    }
}

#[derive(Clone, Copy, Debug, PartialEq, Eq)]
pub(crate) enum ImapPreviewFetchInputError {
    Connection(ImapDiagnosticInputError),
    UidValidity,
    Uids,
}
...
pub(crate) struct ImapInboxPage {
    ...
    pub(crate) remote_message_count: u32,
...
#[derive(Clone, Debug, PartialEq, Eq)]
pub(crate) struct ImapMessagePreview {
    pub(crate) uid: NonZeroU32,
...
async fn diagnose_with_connector(
    ...
    .await?;
    let capabilities = require_imap_capability(&mut session, login_capabilities, deadline).await?;
    identify_client_if_supported(&mut session, &capabilities, deadline).await?;
...
async fn require_imap_capability(
    ...
    login_capabilities: Option<Capabilities>,
    deadline: Instant,
) -> Result<Capabilities, ImapDiagnosticFailure> {
...
async fn identify_client_if_supported(
    session: &mut AuthenticatedSession,
    ...
    .await
    {
        ...
        Err(_) => Err(timeout_failure(ImapDiagnosticStage::Capability)),
    }
}

fn requires_client_id_before_mailbox(host: &str) -> bool {
    host.trim_end_matches('.')
        .eq_ignore_ascii_case("imap.163.com")
}
...
pub(crate) async fn fetch_imap_message_previews(
    request: ImapPreviewFetchRequest,
) -> Result<Box<[ImapMessagePreview]>, ImapInboxFetchFailure> {
    let connector = platform_connector().map_err(map_diagnostic_failure)?;
    fetch_imap_message_previews_with_connector(request, connector, InboxFetchLimits::production())
        .await
}

async fn fetch_imap_message_previews_with_connector(
    request: ImapPreviewFetchRequest,
    connector: TlsConnector,
    limits: InboxFetchLimits,
) -> Result<Box<[ImapMessagePreview]>, ImapInboxFetchFailure> {
    let deadline = Instant::now() + INBOX_FETCH_TIMEOUT;
    let ImapPreviewFetchRequest {
        host,
        port,
        login,
        secret,
    ...
    if let Some(mut session) =
        take_most_recent_cached_inbox_session(host.as_ref(), port, login.as_ref(), uid_validity)
    {
        match fetch_previews(&mut session, &uids, deadline, limits).await {
            Ok(previews) => {
                cache_inbox_session(host.as_ref(), port, login.as_ref(), uid_validity, session);
                return Ok(previews);
            }
    ...
    let mut session = open_inbox_body_session(
        host.as_ref(),
        port,
        login.as_ref(),
        &secret,
    ...
    .await?;
    let previews = fetch_previews(&mut session, &uids, deadline, limits).await?;
    cache_inbox_session(host.as_ref(), port, login.as_ref(), uid_validity, session);
    Ok(previews)
}
...
async fn fetch_canonical_inbox_inner(
    ...
    let mut mailbox_from_status = false;
    let (mut session, mut mailbox, prewarmed_sessions) = if let Some(mut cached) = cached_session {
        ...
        match probe_result {
            Ok(mailbox) => {
                mailbox_from_status = true;
                (cached, mailbox, Vec::new())
            }
            Err(failure) if recoverable_cached_session_failure(failure) => {
    ...
    if !mailbox_from_status && mailbox.uid_next.and_then(NonZeroU32::new).is_none() {
        let status = session.status_inbox(deadline).await?;
        if let (Some(examined), Some(current)) = (mailbox.uid_validity, status.uid_validity)
            && examined != current
        {
            return Err(ImapInboxFetchFailure::UidValidityChanged {
                expected: examined,
        ...
        mailbox.uid_validity = status.uid_validity.or(mailbox.uid_validity);
        mailbox.uid_next = status.uid_next;
    }
    if mailbox.uid_next.and_then(NonZeroU32::new).is_none() {
        let highest = session.uid_search(b"UID *", deadline).await?;
        if highest.len() > 1 {
            return Err(ImapInboxFetchFailure::Protocol);
        }
        mailbox.uid_next = Some(match highest.first().copied() {
            Some(uid) => uid
                .checked_add(1)
    ...
    if mode == InboxFetchMode::Metadata && initial_snapshot && mailbox.exists > 0 {
        ...
        return Ok(ImapInboxPage {
            uid_validity,
            uid_next,
            remote_message_count: mailbox.exists,
...
async fn fetch_previews(
    ...
    let uid_set = uids
        ...
        .join(",");
    let command = format!(
        "UID FETCH {uid_set} (UID BODY.PEEK[HEADER.FIELDS (MIME-VERSION CONTENT-TYPE CONTENT-TRANSFER-ENCODING)] BODY.PEEK[TEXT]<0.{MAX_METADATA_PREVIEW_WIRE_BYTES}>)"
    );
    let id = session.run_command(command, deadline).await?;
    let mut previews = Vec::with_capacity(uids.len());
    loop {
        match read_receive_response(session, deadline).await? {
            ...
            ReceiveResponse::Fetch(fetch) => {
                if previews.len() == limits.max_messages {
                    return Err(ImapInboxFetchFailure::ResourceLimit);
                }
                let uid = fetch
                    .uid
                ...
                if !uids.contains(&uid)
                    || previews
                        .iter()
                        .any(|preview: &ImapMessagePreview| preview.uid == uid)
                {
                    return Err(ImapInboxFetchFailure::Protocol);
                }
                let text = match (&fetch.metadata_headers, &fetch.metadata_body_text) {
                    (Some(headers), Some(body)) => project_metadata_preview(headers, body),
                    _ => Box::default(),
                };
                previews.push(ImapMessagePreview { uid, text });
            }
            ReceiveResponse::Bye => return Err(ImapInboxFetchFailure::Offline),
            ReceiveResponse::Other => {}
        }
    }
    previews.sort_unstable_by_key(|preview| preview.uid);
    Ok(previews.into_boxed_slice())
}
...
async fn fetch_message_contents(
    ...
    for metadata in included {
        let declared_bytes = usize::try_from(metadata.declared_bytes)
            .map_err(|_| ImapInboxFetchFailure::ResourceLimit)?;
        let (content, preview) = if declared_bytes > limits.max_message_literal_bytes {
            (
            ...
            let preview = MessageParser::default()
                .parse(&body.bytes)
                .and_then(|message| message.body_preview(MAX_METADATA_PREVIEW_BYTES))
                .map(|text| clean_preview_text(&text))
                .unwrap_or_default();
            (ImapInboxContent::Fetched(body.bytes), preview)
        };
...
struct ProjectedFetch {
    ...
    metadata_body_text: Option<Box<[u8]>>,
    body: Option<Box<[u8]>>,
}
...
fn project_protocol_fetch_item(
    ...
    match item {
                ...
                ) =>
        {
            if let Some(bytes) = data.decoded() {
                if bytes.len() > MAX_METADATA_HEADER_BYTES {
                    #[cfg(feature = "bench-harness")]
                    eprintln!(
                        "NIVALIS_PERF imap_metadata_header_limit bytes={} maximum={MAX_METADATA_HEADER_BYTES}",
                        bytes.len()
                    );
                    return Err(ImapInboxFetchFailure::ResourceLimit);
                }
        ...
        FetchResponseItem::BodySection {
            section,
            origin,
            data,
        } if section.parts().next().is_none()
            && section.text() == Some(FetchSectionText::Text)
            && origin.is_none_or(|origin| origin == 0) =>
        {
            if let Some(bytes) = data.decoded() {
                let retained_bytes = bytes.len().min(MAX_PROVIDER_PREVIEW_FALLBACK_BYTES);
                if bytes.len() > MAX_METADATA_PREVIEW_WIRE_BYTES {
                    #[cfg(feature = "bench-harness")]
                    eprintln!(
                        "NIVALIS_PERF imap_preview_partial_ignored bytes={} requested={MAX_METADATA_PREVIEW_WIRE_BYTES} retained={retained_bytes}",
                        bytes.len(),
                    );
                }
                let bytes = &bytes[..retained_bytes];
                set_once(&mut fetch.metadata_body_text, bytes.into())?;
            }
        }
...
fn parse_metadata(fetch: ProjectedFetch) -> Result<InboxMetadata, ImapInboxFetchFailure> {
    ...
    let preview = match (&fetch.metadata_headers, &fetch.metadata_body_text) {
        (Some(headers), Some(body)) => project_metadata_preview(headers, body),
        _ => Box::default(),
    };
...
fn project_metadata_preview(headers: &[u8], body: &[u8]) -> Box<str> {
    let mut raw = Vec::with_capacity(headers.len().saturating_add(body.len()).saturating_add(4));
    raw.extend_from_slice(headers);
    if !headers.ends_with(b"\r\n\r\n") && !headers.ends_with(b"\n\n") {
        if headers.ends_with(b"\r\n") {
            raw.extend_from_slice(b"\r\n");
        } else if headers.ends_with(b"\n") {
            raw.extend_from_slice(b"\n");
        } else {
            raw.extend_from_slice(b"\r\n\r\n");
        }
    ...
    MessageParser::default()
        .parse(&raw)
        .and_then(|message| message.body_preview(MAX_METADATA_PREVIEW_BYTES))
        .map(|preview| clean_preview_text(&preview))
        .filter(|preview| !preview.is_empty())
        .unwrap_or_default()
}

fn clean_preview_text(raw: &str) -> Box<str> {
    let mut selected = String::with_capacity(raw.len().min(MAX_METADATA_PREVIEW_BYTES));
    for line in raw.lines() {
        ...
        let quoted = line.starts_with('>')
            ...
            || (lower.starts_with("from:") && !selected.is_empty())
            || (line.starts_with("发件人：") && !selected.is_empty());
        if quoted {
        ...
        for character in line.chars() {
            if character.is_whitespace() || character.is_control() {
                pending_space = !selected.is_empty();
                continue;
            }
            if pending_space && !selected.ends_with(' ') {
                selected.push(' ');
            }
            pending_space = false;
            if selected.chars().count() == MAX_PREVIEW_CHARACTERS
                || selected.len() + character.len_utf8() > MAX_METADATA_PREVIEW_BYTES
            {
                return selected.trim_end().to_owned().into_boxed_str();
            }
            selected.push(character);
        }
    }
    selected.trim().to_owned().into_boxed_str()
}
...
fn map_receive_io_error(error: &io::Error) -> ImapInboxFetchFailure {
    #[cfg(feature = "bench-harness")]
    eprintln!("NIVALIS_PERF imap_receive_io_error kind={:?}", error.kind());
    match error.kind() {
...
mod tests {
    ...
    async fn write_capabilities(
        ...
        stream
            .write_all(if supports_id {
                b"* CAPABILITY IMAP4rev1 IDLE ID\r\n"
            } else {
    ...
    async fn run_script(stream: &mut TlsStream<TcpStream>, script: Script) -> Vec<Box<str>> {
        ...
        match script {
            ...
            Script::Success | Script::ClientIdRequired | Script::MailboxDenied => {}
        ...
        write_capabilities(
            stream,
            &capability,
            matches!(script, Script::ClientIdRequired),
        )
        .await;
        commands.push(capability);

        if matches!(script, Script::ClientIdRequired) {
            let client_id = required_command(stream).await;
            assert_eq!(command_name(&client_id), Some("ID"));
            assert!(client_id.contains("\"name\" \"Nivalis\""));
            stream
                .write_all(b"* ID (\"name\" \"Nivalis fixture\")\r\n")
                .await
                .unwrap();
            stream
                .write_all(&tagged_response(&client_id, "OK client identified"))
                .await
                .unwrap();
            commands.push(client_id);
        }
    ...
    #[test]
    fn identifies_the_client_before_opening_a_provider_mailbox() {
        run_async(async {
            let server = spawn_server(Script::ClientIdRequired).await;
            let result = diagnose_with_connector(
                request(server.address.port()),
                server.connector,
                DiagnosticLimits::production(),
            ...
            let names: Vec<_> = commands
                ...
                .filter_map(|command| command_name(command))
                .collect();
            assert_eq!(names, ["LOGIN", "CAPABILITY", "ID", "EXAMINE", "LOGOUT"]);
        });
    }
    ...
    #[test]
    fn identifies_163_before_mailbox_selection_without_suffix_confusion() {
        assert!(requires_client_id_before_mailbox("imap.163.com"));
        assert!(requires_client_id_before_mailbox("IMAP.163.COM."));
        assert!(!requires_client_id_before_mailbox("imap.example.test"));
        assert!(!requires_client_id_before_mailbox(
            "imap.163.com.example.test"
        ));
    }
}
...
mod inbox_fetch_tests {
    ...
    struct ServerPlan {
        ...
        ignore_preview_partial: bool,
        preview_content_type: &'static str,
    ...
    impl ServerPlan {
        fn messages(messages: Vec<FixtureMessage>) -> Self {
            Self {
                ...
                omit_uid_next: false,
                status_supplies_uid_next: false,
    ...
    #[test]
    fn metadata_preview_decodes_transfer_encoding_and_html() {
        let quoted_printable_headers = b"Content-Type: text/plain; charset=utf-8\r\nContent-Transfer-Encoding: quoted-printable\r\n\r\n";
        assert_eq!(
            project_metadata_preview(
                quoted_printable_headers,
                b"=E9=82=AE=E4=BB=B6=E6=AD=A3=E6=96=87=E9=A2=84=E8=A7=88"
            )
            .as_ref(),
            "邮件正文预览"
        );

        let html_headers = b"MIME-Version: 1.0\r\nContent-Type: text/html; charset=utf-8\r\n\r\n";
        assert_eq!(
            project_metadata_preview(
                html_headers,
                b"<html><body><p>Hello <strong>preview</strong>.</p></body></html>"
            )
            .as_ref(),
            "Hello preview."
        );
        ...
        assert_eq!(
            project_metadata_preview(base64_headers, b"6YKu5Lu25q2j5paH6aKE6KeI").as_ref(),
            "邮件正文预览"
        );
    }

    #[test]
    fn metadata_preview_extracts_text_from_multipart_body() {
        let headers = b"MIME-Version: 1.0\r\nContent-Type: multipart/alternative; boundary=preview-boundary\r\n\r\n";
        let body = b"--preview-boundary\r\nContent-Type: text/plain; charset=utf-8\r\n\r\nFast local preview.\r\n--preview-boundary\r\nContent-Type: text/html; charset=utf-8\r\n\r\n<p>HTML fallback.</p>\r\n--preview-boundary--\r\n";

        assert_eq!(
            project_metadata_preview(headers, body).as_ref(),
            "Fast local preview."
        );
    }
    ...
    async fn write_metadata(
        ...
        let body_offset = message
            ...
            .map_or(message.raw.len(), |offset| offset + 4);
        let body_end = if ignore_preview_partial {
            message.raw.len()
        } else {
            message
                .raw
                .len()
                .min(body_offset + MAX_METADATA_PREVIEW_WIRE_BYTES)
        };
        let body = &message.raw[body_offset..body_end];
        let headers = format!(
        ...
        let response_head = if include_preview {
            format!(
                "* {sequence} FETCH (UID {} FLAGS (\\Seen \\Flagged) INTERNALDATE \"20-Jul-2026 12:00:00 +0000\" RFC822.SIZE {} BODY[HEADER.FIELDS (DATE FROM SUBJECT MESSAGE-ID MIME-VERSION CONTENT-TYPE CONTENT-TRANSFER-ENCODING)] {{{}}}\r\n{} BODY[TEXT]<0> {{{}}}\r\n",
                message.uid,
    ...
    async fn run_server(stream: ServerTlsStream<TcpStream>, mut plan: ServerPlan) -> Vec<Box<str>> {
        ...
        stream
            .get_mut()
            .write_all(if plan.require_client_id {
                b"* OK [CAPABILITY IMAP4rev1 ID] receive fixture ready\r\n".as_slice()
            } else {
                b"* OK receive fixture ready\r\n".as_slice()
            })
            .await
            .unwrap();
        ...
        tagged(
            &mut stream,
            &login,
            if plan.require_client_id {
                "OK [CAPABILITY IMAP4rev1 ID] authenticated"
            } else if plan.omit_status_response_text {
        ...
        let mut commands = vec![login];
        if plan.require_client_id {
            let Some(client_id) = read_command(&mut stream).await else {
                return commands;
            };
            assert_eq!(command_name(&client_id), Some("ID"));
            assert!(client_id.contains("\"name\" \"Nivalis\""));
            stream
                ...
                .await
                .unwrap();
            tagged(&mut stream, &client_id, "OK client identified").await;
            commands.push(client_id);
        }
        ...
        if plan.omit_uid_next {
            let Some(status) = read_command(&mut stream).await else {
                return commands;
            };
            assert_eq!(command_name(&status), Some("STATUS"));
            let uid_next_status = if plan.status_supplies_uid_next {
                format!(" UIDNEXT {uid_next}")
            } else {
                String::new()
            };
            stream
                ...
                .write_all(
                    format!(
                        "* STATUS INBOX (MESSAGES {exists} UIDVALIDITY {}{uid_next_status})\r\n",
                        plan.uid_validity.unwrap_or(UID_VALIDITY)
                    )
                    .as_bytes(),
                )
                .await
                .unwrap();
            tagged(&mut stream, &status, "OK status complete").await;
            commands.push(status);
            if !plan.status_supplies_uid_next {
                let Some(highest_uid) = read_command(&mut stream).await else {
                    return commands;
                };
                assert!(highest_uid.ends_with(" UID SEARCH UID *"));
                if let Some(uid) = plan.messages.last().map(|message| message.uid) {
                    stream
                        .get_mut()
                    ...
                    stream.get_mut().write_all(b"* SEARCH\r\n").await.unwrap();
                }
                tagged(&mut stream, &highest_uid, "OK search complete").await;
                commands.push(highest_uid);
            }
        }
        ...
        for message in requested_messages {
            ...
            write_metadata(
                ...
                sequence,
                message,
                metadata.contains("BODY.PEEK[TEXT]"),
                plan.ignore_preview_partial,
                plan.preview_content_type,
            )
            .await;
        }
        ...
        while let Some(command) = read_command(&mut stream).await {
            ...
            if command.contains("BODY.PEEK[TEXT]") {
                ...
                tagged(&mut stream, &command, "OK preview complete").await;
                continue;
            }
    ...
    async fn run_metadata_only_server(
        ...
        while let Some(command) = read_command(&mut stream).await {
            ...
            assert!(!command.contains("BODY.PEEK[TEXT]"));
    ...
    #[test]
    fn metadata_sync_returns_without_waiting_for_preview_bodies() {
        run_async(async {
            ...
            assert!(
                page.messages
                    .iter()
                    .all(|message| message.preview.is_empty())
            );
            ...
            assert!(
                commands
                    .iter()
                    .all(|command| !command.contains("BODY.PEEK[TEXT]"))
            );
        });
    }

    #[test]
    fn preview_fetch_reuses_metadata_session_and_hydrates_only_requested_uids() {
        run_async(async {
            let server = spawn_server(ServerPlan::messages(vec![
                fixture(1, RAW_ONE),
                fixture(2, RAW_TWO),
            ...
            let request = ImapPreviewFetchRequest::new(
                HOST,
                server.address.port(),
                LOGIN,
                Secret::new(PASSWORD.to_vec()).unwrap(),
            ...
            .unwrap();
            let previews = fetch_imap_message_previews_with_connector(
                request,
                server.connector.clone(),
                limits(),
            )
            ...
            assert!(!fetches[0].contains("BODY.PEEK[TEXT]"));
            assert!(fetches[1].contains("UID FETCH 2"));
            assert!(fetches[1].contains("BODY.PEEK[TEXT]"));
        });
    }

    #[test]
    fn preview_fetch_bounds_a_provider_that_ignores_the_partial_length() {
        run_async(async {
            let mut raw = b"From: Sender <sender@example.test>\r\n\
Subject: Provider ignored partial\r\n\
MIME-Version: 1.0\r\n\
Content-Type: text/html; charset=utf-8\r\n\
\r\n\
<html><body><!--"
                .to_vec();
            raw.extend(std::iter::repeat_n(b'x', 9 * 1024));
            raw.extend_from_slice(
                b"--><p>Visible provider preview after the requested range.</p></body></html>",
            );
            raw.extend(std::iter::repeat_n(b'x', 3 * 1024));
            let mut plan = ServerPlan::messages(vec![fixture(1, &raw)]);
            plan.ignore_preview_partial = true;
            plan.preview_content_type = "text/html; charset=utf-8";
            ...
            .unwrap();
            let tolerant_limits = InboxFetchLimits {
                max_message_literal_bytes: 16 * 1024,
                max_page_literal_bytes: 16 * 1024,
                ..limits()
            };
            let previews = fetch_imap_message_previews_with_connector(
                request,
            ...
            assert!(
                previews[0]
                    .text
                    .starts_with("Visible provider preview after the requested range."),
                "unexpected preview: {:?}",
                previews[0].text
            );
            assert!(previews[0].text.len() <= MAX_METADATA_PREVIEW_BYTES);
            ...
            assert!(
                commands
                    .iter()
                    .any(|command| command.contains("BODY.PEEK[TEXT]<0.8192>"))
            );
        });
    }
    ...
    fn fetches_a_bounded_page_and_handles_an_empty_inbox() {
        run_async(async {
            ...
            assert_eq!(page.messages[0].preview.as_ref(), "Hello one.");
            assert_eq!(
    ...
    #[test]
    fn identifies_the_client_before_the_fast_provider_mailbox_pipeline() {
        run_async(async {
            let mut plan = ServerPlan::messages(vec![fixture(1, RAW_ONE)]);
            plan.require_client_id = true;
            let server = spawn_server(plan).await;
    ...
    #[test]
    fn rejects_missing_uidvalidity_or_changed_epoch() {
        run_async(async {
    ...
    #[test]
    fn recovers_missing_examine_uidnext_from_bounded_status() {
        run_async(async {
            let mut plan = ServerPlan::messages(vec![fixture(1, RAW_ONE)]);
            plan.omit_uid_next = true;
            plan.status_supplies_uid_next = true;
            let server = spawn_server(plan).await;
            ...
            assert_eq!(
                commands
                    ...
                    .take(3)
                    .collect::<Vec<_>>(),
                ["LOGIN", "EXAMINE", "STATUS"]
            );
        });
    }

    #[test]
    fn infers_missing_provider_uidnext_from_one_bounded_uid_search() {
        run_async(async {
            let mut plan = ServerPlan::messages(vec![fixture(41, RAW_ONE)]);
            plan.omit_uid_next = true;
            let server = spawn_server(plan).await;
            ...
            assert_eq!(page.uid_next.get(), 42);
            assert_eq!(page.messages.len(), 1);
            assert_eq!(page.messages[0].uid.get(), 41);
            let commands = finish(server).await;
            assert!(
                commands
                    .iter()
                    .any(|command| command.ends_with(" UID SEARCH UID *"))
            );
        });
    }
