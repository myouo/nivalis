...
// Nivalis fetches at most one 1 MiB raw message per IMAP literal. Reject the
// declaration before nom reports Needed::Size and async-imap grows its buffer.
const MAX_LITERAL_BYTES: u32 = 1024 * 1024;
...
mod tests {
    ...
    #[test]
    fn literal_limit_accepts_n_and_rejects_n_plus_one_before_payload() {
        let mut exact = format!("{{{MAX_LITERAL_BYTES}}}\r\n").into_bytes();
        exact.resize(exact.len() + MAX_LITERAL_BYTES as usize, b'x');
        assert_eq!(literal(&exact).unwrap().1.len(), MAX_LITERAL_BYTES as usize);

        let oversized = format!("{{{}}}\r\n", MAX_LITERAL_BYTES + 1);
        assert_matches!(
            literal(oversized.as_bytes()),
